...
class AddSellStWindow(QMainWindow):
    # 定义信号（用于通知主窗口保存完成）
    ...
    def __init__(self, parent=None):
        ...
        # 初始化策略数据
        self.add_sell_st_stock_list = []
        self.add_sell_st_file_list = []
    ...
    def init_ui(self):
        """初始化界面"""
        self.setWindowTitle("添加卖出策略（多批次独立配置，最多4批）")
    ...
    def create_stock_single_tab(self):
        ...
        self.sell_icon_label = QLabel()
        pixmap = QPixmap("image/sell.png")
        if pixmap.isNull():
            self.sell_icon_label.setText("")
            self.sell_icon_label.setAlignment(Qt.AlignCenter)
        else:
            self.sell_icon_label.setPixmap(pixmap.scaled(81, 91, Qt.KeepAspectRatio))
        self.sell_icon_label.setFixedSize(81, 91)
        ...
        step1_layout.addWidget(self.sell_icon_label)
    ...
    def create_stock_folder_tab(self):
        ...
        self.file_sell_icon_label = QLabel()
        pixmap = QPixmap("image/sell.png")
        if pixmap.isNull():
            self.file_sell_icon_label.setText("")
            self.file_sell_icon_label.setAlignment(Qt.AlignCenter)
        else:
            self.file_sell_icon_label.setPixmap(pixmap.scaled(81, 91, Qt.KeepAspectRatio))
        self.file_sell_icon_label.setFixedSize(81, 91)
        ...
        step1_layout.addStretch()
        step1_layout.addWidget(self.file_sell_icon_label)
        main_layout.addLayout(step1_layout)
    ...
    def reset_stock_tab(self):
        ...
        self.add_sell_st_stock_list.clear()
    ...
    def reset_file_tab(self):
        ...
        self.add_sell_st_file_list.clear()
    ...
    def save_stock_st(self):
        ...
        for batch_idx in range(4):
            ...
            self.add_sell_st_stock_list.append(batch_st)

        QMessageBox.information(self, "保存成功", "策略已经保存，请按需要进行运行")
        self.st_saved.emit(self.add_sell_st_stock_list, [])
        self.close()
    ...
    def save_file_st(self):
        ...
        for batch_idx in range(4):
            ...
            self.add_sell_st_file_list.append(batch_st)

        QMessageBox.information(self, "保存成功", "策略已经保存，请按需要进行运行")
        self.st_saved.emit([], self.add_sell_st_file_list)
        self.close()
