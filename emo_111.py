...
class emo_doctor:
    def __init__(self):
        self.data_fetcher = ak_data.data_his()
        ...
        self.emo_columns = [
            ...
            '连板晋级率', '跌停数量', '炸板数量', '炸板率',
            '通达信热股上涨率', '通达信热股下跌率', '通达信热股平均涨幅',
            '上涨大于7比率', '上涨5-7比率', '上涨3-5比率', '上涨0-3比率',
            '下跌0-3比率', '下跌3-5比率', '下跌5-7比率', '下跌大于7比率',
            '全市场平均涨幅', '上涨比例', '下跌比例',
            '昨龙虎榜平均涨幅', '昨龙虎榜上涨比率', '昨龙虎榜下跌比率',
    ...
    def cal_emo_indicators(self):
        ...
        # 第6-8项（使用缓存的rate_df）
        # ========================================
        print(" 第六项：分析通达信热股 🔵 处理通达信热股...")
        try:
            if rate_df is not None and not rate_df.empty:
                print("  🔵 读取tdx_hot.txt...")
                hot_codes_raw = []
                hot_file = os.path.join(self.data_dir, "tdx_hot.txt")

                if os.path.exists(hot_file):
                    try:
                        with open(hot_file, 'r', encoding='utf-8') as f:
                            hot_codes_raw = [line.strip() for line in f if line.strip()]
                        print(f"  ✅ 读取热股文件: {len(hot_codes_raw)} 只")
                    except Exception as e:
                        print(f"  ❌ 读取热股文件失败: {e}")
                else:
                    print(f"  ⚠️ 热股文件不存在: {hot_file}")

                hot_codes_clean = [code.split('.')[0].zfill(6) for code in hot_codes_raw]
                hot_codes_set = set(hot_codes_clean)

                if hot_codes_set and '代码' in rate_df.columns and '涨幅' in rate_df.columns:
                    print("  🔵 匹配热股数据...")
                    rate_df['代码_match'] = rate_df['代码'].astype(str).str.zfill(6)
                    df_matched = rate_df[rate_df['代码_match'].isin(hot_codes_set)].copy()
                    print(f"  ✅ 匹配到 {len(df_matched)} 只热股")

                    if not df_matched.empty:
                        df_matched['涨幅'] = pd.to_numeric(df_matched['涨幅'], errors='coerce')
                        df_matched = df_matched.dropna(subset=['涨幅'])

                        if '时间' in df_matched.columns and not df_matched['时间'].empty:
                            time_stats = df_matched.groupby('时间').agg(
                                通达信热股上涨家数=('涨幅', lambda x: (x >= 0).sum()),
                                通达信热股下跌家数=('涨幅', lambda x: (x < 0).sum()),
                                通达信热股总家数=('涨幅', 'count'),
                                通达信热股平均涨幅=('涨幅', 'mean')
                            ).reset_index()

                            time_stats['通达信热股上涨率'] = time_stats.apply(
                                lambda row: round((row['通达信热股上涨家数'] / row['通达信热股总家数']) * 100, 2)
                                if row['通达信热股总家数'] > 0 else 0,
                                axis=1
                            )
                            time_stats['通达信热股下跌率'] = time_stats.apply(
                                lambda row: round((row['通达信热股下跌家数'] / row['通达信热股总家数']) * 100, 2)
                                if row['通达信热股总家数'] > 0 else 0,
                                axis=1
                            )
                            time_stats['通达信热股平均涨幅'] = time_stats['通达信热股平均涨幅'].round(2)
                            time_stats = time_stats.sort_values('时间')

                            for idx, row in time_stats.iterrows():
                                time_point = row['时间']
                                update_temp_data(time_point, {
                                    '通达信热股上涨率': float(row['通达信热股上涨率']),
                                    '通达信热股下跌率': float(row['通达信热股下跌率']),
                                    '通达信热股平均涨幅': float(row['通达信热股平均涨幅']),
                                    '通达信热股上涨家数': int(row['通达信热股上涨家数']),
                                    '通达信热股下跌家数': int(row['通达信热股下跌家数']),
                                    '通达信热股总家数': int(row['通达信热股总家数'])
                                })

                            if not time_stats.empty:
                                latest = time_stats.iloc[-1]
                                self.indicators['通达信热股上涨率'] = latest['通达信热股上涨率']
                                self.indicators['通达信热股下跌率'] = latest['通达信热股下跌率']
                                self.indicators['通达信热股平均涨幅'] = latest['通达信热股平均涨幅']
                            else:
                                self.indicators['通达信热股上涨率'] = 0
                                self.indicators['通达信热股下跌率'] = 0
                                self.indicators['通达信热股平均涨幅'] = 0
                        else:
                            self.indicators['通达信热股上涨率'] = 0
                            self.indicators['通达信热股下跌率'] = 0
                            self.indicators['通达信热股平均涨幅'] = 0
        ...
        print(" 第七项：分析全市场涨幅分布 🔵 处理全体股票行情（第7项，数据量大，可能较慢）...")
        try:
            if rate_df is not None and not rate_df.empty:
                rate_df['涨幅'] = pd.to_numeric(rate_df['涨幅'], errors='coerce')
                rate_df = rate_df.dropna(subset=['涨幅'])

                if '时间' in rate_df.columns and not rate_df['时间'].empty:
                    print(f"  🔵 开始分组统计，共 {len(rate_df)} 条数据，{rate_df['时间'].nunique()} 个时间点...")
                    time_stats = rate_df.groupby('时间').agg(
                        涨幅大于7数量=('涨幅', lambda x: (x >= 7).sum()),
                        涨幅5_7数量=('涨幅', lambda x: ((x >= 5) & (x < 7)).sum()),
                        涨幅3_5数量=('涨幅', lambda x: ((x >= 3) & (x < 5)).sum()),
                        涨幅0_3数量=('涨幅', lambda x: ((x >= 0) & (x < 3)).sum()),
                        跌幅0_3数量=('涨幅', lambda x: ((x < 0) & (x > -3)).sum()),
                        跌幅3_5数量=('涨幅', lambda x: ((x <= -3) & (x > -5)).sum()),
                        跌幅5_7数量=('涨幅', lambda x: ((x <= -5) & (x > -7)).sum()),
                        跌幅小于7数量=('涨幅', lambda x: (x <= -7).sum()),
                        上涨家数=('涨幅', lambda x: (x >= 0).sum()),
                        下跌家数=('涨幅', lambda x: (x < 0).sum()),
                        总家数=('涨幅', 'count'),
                        平均涨幅=('涨幅', 'mean')
                    ).reset_index()
                    print("  ✅ 分组统计完成")

                    time_stats['上涨大于7比率'] = time_stats.apply(
                        lambda row: round((row['涨幅大于7数量'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['上涨5-7比率'] = time_stats.apply(
                        lambda row: round((row['涨幅5_7数量'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['上涨3-5比率'] = time_stats.apply(
                        lambda row: round((row['涨幅3_5数量'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['上涨0-3比率'] = time_stats.apply(
                        lambda row: round((row['涨幅0_3数量'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['下跌0-3比率'] = time_stats.apply(
                        lambda row: round((row['跌幅0_3数量'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['下跌3-5比率'] = time_stats.apply(
                        lambda row: round((row['跌幅3_5数量'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['下跌5-7比率'] = time_stats.apply(
                        lambda row: round((row['跌幅5_7数量'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['下跌大于7比率'] = time_stats.apply(
                        lambda row: round((row['跌幅小于7数量'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['上涨比例'] = time_stats.apply(
                        lambda row: round((row['上涨家数'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['下跌比例'] = time_stats.apply(
                        lambda row: round((row['下跌家数'] / row['总家数']) * 100, 2)
                        if row['总家数'] > 0 else 0,
                        axis=1
                    )
                    time_stats['全市场平均涨幅'] = time_stats['平均涨幅'].round(2)
                    time_stats = time_stats.sort_values('时间')

                    for idx, row in time_stats.iterrows():
                        time_point = row['时间']
                        update_temp_data(time_point, {
                            '上涨大于7比率': float(row['上涨大于7比率']),
                            '上涨5-7比率': float(row['上涨5-7比率']),
                            '上涨3-5比率': float(row['上涨3-5比率']),
                            '上涨0-3比率': float(row['上涨0-3比率']),
                            '下跌0-3比率': float(row['下跌0-3比率']),
                            '下跌3-5比率': float(row['下跌3-5比率']),
                            '下跌5-7比率': float(row['下跌5-7比率']),
                            '下跌大于7比率': float(row['下跌大于7比率']),
                            '全市场平均涨幅': float(row['全市场平均涨幅']),
                            '上涨比例': float(row['上涨比例']),
                            '下跌比例': float(row['下跌比例']),
                            # '涨幅大于7数量': int(row['涨幅大于7数量']),
                            # '涨幅5_7数量': int(row['涨幅5_7数量']),
                            # '涨幅3_5数量': int(row['涨幅3_5数量']),
                            # '涨幅0_3数量': int(row['涨幅0_3数量']),
                            # '跌幅0_3数量': int(row['跌幅0_3数量']),
                            # '跌幅3_5数量': int(row['跌幅3_5数量']),
                            # '跌幅5_7数量': int(row['跌幅5_7数量']),
                            # '跌幅小于7数量': int(row['跌幅小于7数量']),
                            # '上涨家数': int(row['上涨家数']),
                            # '下跌家数': int(row['下跌家数']),
                            # '总家数': int(row['总家数']),
                            # '平均涨幅': float(row['平均涨幅'])
                    ...
                    if not time_stats.empty:
                        latest = time_stats.iloc[-1]
                        self.indicators['上涨大于7比率'] = latest['上涨大于7比率']
                        self.indicators['上涨5-7比率'] = latest['上涨5-7比率']
                        self.indicators['上涨3-5比率'] = latest['上涨3-5比率']
                        self.indicators['上涨0-3比率'] = latest['上涨0-3比率']
                        self.indicators['下跌0-3比率'] = latest['下跌0-3比率']
                        self.indicators['下跌3-5比率'] = latest['下跌3-5比率']
                        self.indicators['下跌5-7比率'] = latest['下跌5-7比率']
                        self.indicators['下跌大于7比率'] = latest['下跌大于7比率']
                        self.indicators['全市场平均涨幅'] = latest['全市场平均涨幅']
                        self.indicators['上涨比例'] = latest['上涨比例']
                        self.indicators['下跌比例'] = latest['下跌比例']
                    else:
                        self._init_stock_distribution()
                else:
                    self._init_stock_distribution()
            else:
                self._init_stock_distribution()
        ...
        print("第八项：分析昨日龙虎榜在今天的表现🔵 处理龙虎榜数据...")
        try:
            if rate_df is not None and not rate_df.empty:
                print("  🔵 读取lhb_last.csv...")
                lhb_file = os.path.join(self.data_dir, 'lhb_last.csv')
                if os.path.exists(lhb_file):
                    lhb_df = pd.read_csv(lhb_file, encoding='utf-8-sig')
                    print(f"  ✅ 读取龙虎榜文件: {len(lhb_df)} 只")
                    if '代码' in lhb_df.columns:
                        lhb_codes = set(lhb_df['代码'].astype(str).str.zfill(6).tolist())
                        if lhb_codes and '代码' in rate_df.columns and '涨幅' in rate_df.columns:
                            print("  🔵 匹配龙虎榜数据...")
                            rate_df['代码_match'] = rate_df['代码'].astype(str).str.zfill(6)
                            df_matched = rate_df[rate_df['代码_match'].isin(lhb_codes)].copy()
                            print(f"  ✅ 匹配到 {len(df_matched)} 只龙虎榜股")
                            ...
                            if not df_matched.empty:
                                ...
                                if '时间' in df_matched.columns and not df_matched['时间'].empty:
                                    ...
                                    time_stats['昨龙虎榜上涨比率'] = time_stats.apply(
                                        lambda row: round((row['上涨家数'] / row['总家数']) * 100, 2)
                                        if row['总家数'] > 0 else 0,
                                        axis=1
                                    )
                                    time_stats['昨龙虎榜下跌比率'] = time_stats.apply(
                                        lambda row: round((row['下跌家数'] / row['总家数']) * 100, 2)
                                        if row['总家数'] > 0 else 0,
                                        axis=1
                                    )
                                    time_stats['昨龙虎榜平均涨幅'] = time_stats['平均涨幅'].round(2)
                                    time_stats = time_stats.sort_values('时间')

                                    for idx, row in time_stats.iterrows():
                                        time_point = row['时间']
                                        update_temp_data(time_point, {
                                            '昨龙虎榜平均涨幅': float(row['昨龙虎榜平均涨幅']),
                                            '昨龙虎榜上涨比率': float(row['昨龙虎榜上涨比率']),
                                            '昨龙虎榜下跌比率': float(row['昨龙虎榜下跌比率'])
                                        })

                                    if not time_stats.empty:
                                        latest = time_stats.iloc[-1]
                                        self.indicators['昨龙虎榜平均涨幅'] = latest['昨龙虎榜平均涨幅']
                                        self.indicators['昨龙虎榜上涨比率'] = latest['昨龙虎榜上涨比率']
                                        self.indicators['昨龙虎榜下跌比率'] = latest['昨龙虎榜下跌比率']
                                    else:
                                        self.indicators['昨龙虎榜平均涨幅'] = 0
                                        self.indicators['昨龙虎榜上涨比率'] = 0
                                        self.indicators['昨龙虎榜下跌比率'] = 0
                                else:
                                    self.indicators['昨龙虎榜平均涨幅'] = 0
                                    self.indicators['昨龙虎榜上涨比率'] = 0
                                    self.indicators['昨龙虎榜下跌比率'] = 0
